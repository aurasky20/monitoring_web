...
pythonSocket.on("connect", () => {
  console.log(
    `🔗 Successfully connected to Python detection server at ${PYTHON_SERVER_URL}`
  );
});
...
pythonSocket.on("log", async (data) => {
  console.log("📝 New detection logged:", data);

  try {
    const detections = await getDetectionsByDate(data.tanggal);
    const stats = await getDetectionStats();
